...
export const forceAuthRefresh = async () => {
  debug('[Auth] forceAuthRefresh: Starting authentication refresh');
  ...
  try {
    // First clear any cached profile data
    ...
    if (data?.session) {
      ...
      try {
        ...
        // Trigger a sync event to other components that might be listening
