...
const SignupPage = () => {
  ...
  const handleSubmitInitialForm = async (e) => {
    ...
    if (!firstName) {
      newErrors.firstName = 'İsim giriniz';
    }
    
    if (!lastName) {
      newErrors.lastName = 'Soyisim giriniz';
    }
    ...
    if (!password) {
      newErrors.password = 'Parola giriniz';
      ...
      <div style={{ position: 'relative' }}>
        ...
        <CheckboxInput 
          ...
          onChange={(e) => setAcceptTerms(e.target.checked)}
            />
        <CheckboxLabel>
          <Link to="/terms-of-use" style={{ color: 'var(--color-primary)' }}>Üyelik sözleşmesini</Link> okudum ve kabul ediyorum.
            </CheckboxLabel>
          </CheckboxContainer>
